import React from "react";

const Getsocial = () => {
  return (
    <>
      <p>Be Social With Me.</p>
      <a href="https://www.facebook.com/sejshirke.5" target="_blank" rel="noreferrer">
        <i className="footer-icons fa fa-facebook-official w3-hover-opacity"></i>
      </a>
      <a href="https://www.instagram.com/sejshirke/" target="_blank" rel="noreferrer">
        <i className="footer-icons fa fa-instagram w3-hover-opacity"></i>
      </a>
      <a href="https://www.linkedin.com/in/shejalshirke" target="_blank" rel="noreferrer">
        <i className="footer-icons fa fa-linkedin w3-hover-opacity"></i>
      </a>
      <a href="https://wa.me/919867110710" target="_blank" rel="noreferrer">
        <i className="footer-icons fa fa-whatsapp w3-hover-opacity"></i>
      </a>
      <a href="https://www.youtube.com/@sejshirke" target="_blank" rel="noreferrer">
        <i className="footer-icons fa fa-youtube w3-hover-opacity"></i>
      </a>
    </>
  );
};

export default Getsocial;
